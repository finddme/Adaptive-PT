...
from sources.early_stopping import Early_Stopping, Early_Stopping_BERT
...
def run(do_train,args):
    ...
    if do_train:
        ...
        model_from_pretrained=AutoModelForPreTraining.from_pretrained("bert-base-uncased")
        
        model = BERT(
                config = config,
                transformer=LogitsAdapter(model_from_pretrained),
                num_tokens=tokenizer.vocab_size,
                mask_ignore_token_ids=[tokenizer.vocab['[CLS]'], tokenizer.vocab['[SEP]']]).to(device)
        train_dataset= load_and_cache_examples(args, tokenizer, evaluate=False) 
        global_step, tr_loss = train(args, train_dataset, model, tokenizer,device)
    else:
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer,device) -> Tuple[int, float]:
    ...
    early_stopping = Early_Stopping_BERT(verbose = True)
    global_step = 0
    epochs_trained = 0
    ...
    for epoch in train_iterator:
        ...
        logger.info('####################### Epoch {}: BERT Pre-Training Start #######################'.format(epoch))
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.zero_grad()
            global_step += 1
            train_iterator.write('------------------------------------------------[BERT Train] Epoch {}/{}, Steps {}/{}------------------------'.format(epoch, args.num_train_epochs, step, len(train_dataloader)))
            train_iterator.write('                                    train loss {}'.format(loss))
            ckpt ="BERT_TRAIN_{}epoch_{}step_loss{}.bin".format(epoch,step,loss)
